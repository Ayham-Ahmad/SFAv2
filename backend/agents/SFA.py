...
from ..utils.clean import extract_final_outputs, clean_tables_schema, clean_tables_schema_for_tableAgent, get_chosen_tables_schema
...
class SFA:
    ...
    async def main(self, request=None):
        ...
        tents_ids = tents_agent.get_relevant_tents(self.user_query, self.db, self.company_id)
        print(f"Choosing the Tents result: {tents_ids}\n")
        if not tents_ids:
            return "Sorry, can you clarify your request, like add the kind of data that you need?"
        ...
        tablesAgent_tables_schema = clean_tables_schema_for_tableAgent(tents_schema)
        tables = tables_agent.get_relevant_tables(self.db, self.user_query, tents_ids, tablesAgent_tables_schema)
        ...
        base_prompt = CreatePrompt.init_prompt(self.user_query, tables_schema)
        scratchpad = ""
        iterations = 0
