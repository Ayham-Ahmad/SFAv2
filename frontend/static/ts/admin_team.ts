...
const tableBody =
  document.querySelector<HTMLTableSectionElement>("#team_table_body");
const addBtn =
  document.querySelector<HTMLButtonElement>("#add_member_btn");
const modal =
  document.querySelector<HTMLDivElement>("#add_member_modal");
const form =
  document.querySelector<HTMLFormElement>("#add_member_form");
const cancelBtn =
  document.querySelector<HTMLButtonElement>("#cancel_add_btn");
...
async function loadCompany(): Promise<void> {
  ...
  if (response.ok) {
    const company: CompanyOut = await response.json();
  ...
  if (response.ok) {
    ...
    tableBody.querySelectorAll(".delete-button").forEach((btn) => {
      btn.addEventListener("click", () => {
        const userId = Number((btn as HTMLElement).dataset.id);
