...
class CompanyCRUD:
    @staticmethod
    def create(db: Session, company_data: Union[dict, BaseModel], admin_data: Union[dict, BaseModel], actor_id: Optional[int] = None) -> Company:
        ...
        record_modification(db, TableName.COMPANIES, new_company.company_id, ActionType.CREATE, actor_id, None)
    ...
    @staticmethod
    def update(db: Session, company_id: int, updated_data: Union[dict, BaseModel], actor_id: Optional[int] = None) -> Optional[Company]:
        ...
        if company_record:
            ...
            record_modification(db, TableName.COMPANIES, company_id, ActionType.UPDATE, actor_id, company_old_snapshot)
            db.commit()
            db.refresh(company_record)
        return company_record

    @staticmethod
    def _delete_all_company_users(db: Session, company_id: int, actor_id: Optional[int]):
    ...
    @staticmethod
    def _delete_all_company_tents(db: Session, company_id: int, actor_id: Optional[int]):
        from .tent_events import TentCRUD # Lazy Import
        tents = db.query(TentDatabase).filter(TentDatabase.company_id == company_id).all()
        for tent in tents:
            TentCRUD.delete(db, tent.db_id, actor_id)

    @staticmethod
    def delete(db: Session, company_id: int, actor_id: Optional[int] = None) -> bool:
        company_record = db.query(Company).filter(Company.company_id == company_id).first()
        if company_record:
            CompanyCRUD._delete_all_company_users(db, company_id, actor_id)
            CompanyCRUD._delete_all_company_tents(db, company_id, actor_id)
            
            record_modification(db, TableName.COMPANIES, company_id, ActionType.DELETE, actor_id, get_current_snapshot(company_record))
